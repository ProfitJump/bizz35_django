...
# Import models from dashboard and stripe apps and give them a reference name.
...
# Import stripe api client to work with the stripe API. The stripe API key is set via the django settings module, but held in the .env
...
def create_checkout_session(request):
    if request.method == 'GET':
        ...
        try:
            ...
            # Get the and link the object from the Stipe model in the stripe model.py file ane return the stored stripe customer ID to pass
            # into the checkout creation.
            get_customer_id = StripeProfile.objects.get(user_id=user_id)
            strip_id = get_customer_id.customer_id

            # Begin the checkout session for stripe.
            checkout_session = stripe.checkout.Session.create(
                # pass the current user.id through to stripe to connect the two systems together using client_reference_id. If user.id is
                # none then none will be passes.
                client_reference_id=request.user.id if request.user.is_authenticated else None,
                # This is the URL that will be sent upon a checkout success status.
                success_url=domain_url + 'stripe/success/{CHECKOUT_SESSION_ID}',
            ...
            if not get_profile.is_booster:
                ...
                if commission_matrix <= 1:
                    ...
                    if commission_matrix == 6:
                        commission_owner.profile.boosters = commission_owner.boosters + 4
                        commission_owner.profile.save()
...
class StripeAuthorizeView(View):
    def get(self, request):
        ...
        domain_url = settings.STRIPE_DOMAIN
        url = domain_url + 'stripe/oauth/callback'
        try:
            account_link = stripe.AccountLink.create(
                account=account_id,
                refresh_url=domain_url + 'stripe/oauth/callback',
                return_url=domain_url + 'stripe/oauth/callback',
